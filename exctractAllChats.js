(async function () {
	async function loadChatBox(person) {
		s = await page.$("button._3e4VU");
		await s.click();
	...
	function loadPerson(person) {
		function openChat(person) {
			...
			var personTab = [
				...
				person.test(child.querySelector("span[title]").innerText)
			);
			return personTab;
		}
		return openChat(person);
	}
	// const useHereSelector = ".S7_rT.FV2Qy";
	async function readChat() {
		...
		function image(chatitem) {
			var list = new Set();
			return new Promise((res, rej) => {
				var thumbs = chatitem.querySelectorAll("img._39rvu._20GbR");
				var expectedPics = thumbs.length;
				for (var node of thumbs) {
					var img = node.parentNode.parentNode.querySelector(
						'img[class="_39rvu"]'
					);
					if (img) {
						list.add(img.src);
					} else {
						var observer = new MutationObserver((mts) => {
							for (let ms of mts) {
								if ((ms.type = "childList" && ms.addedNodes.length)) {
									console.log("mutation", ms);
									var img = ms.target.querySelector('img[class="_39rvu"]');
									if (img) {
										list.add(img.src);
										observer.disconnect();
										if (expectedPics == list.size) res(list);
									}
								}
							}
						});
						observer.observe(node.parentNode.parentNode, {
							childList: true,
							subtree: true,
						});
					}
				...
				// 		"chatitem",
				// 		chatitem,
				// 		chatitem.querySelectorAll('img[class="_39rvu"]')
				// 	);

				// 	var result = [...chatitem.querySelectorAll("img._39rvu._20GbR")]
				// 		.filter((el) => el.src.startsWith("blob"))
				// 		.map((el) => el.src);
				// 	res(result);
				// }, 10);
			});
		...
		async function getEBills() {
			var chatBox = getChatPage();
			var chats = [];
			for await (let chat of readChats(chatBox)) {
				await wait(500);
				var img = await image(chat);
				if (!img.size) continue;
				for (let src of img) {
					var imd = await convertURIToImageData(src);

					console.log("= img ===>", imd, img);
					var qr = await jsQR(imd.data, imd.width, imd.height);
					chats.push(qr?.data);
				}
			}
			return chats;
			return new Promise((res, rej) => {
				var now = Date.now();
				var interval = setInterval(() => {
					if (chats.length == count) {
						res(chats);
						clearInterval(interval);
					} else if (Date.now() - now >= 50000000) {
						rej("Tim Out");
						clearInterval(interval);
					}
				}, 0);
		...
		async function getChatText() {
			...
			var chats = [];
			for await (let chat of readChats(chatBox)) {
				chats.push(extractChatText(chat));
			}
...
`);
	...
	if (process.env.OS) require("dotenv").config();
	var puppeteer = require("puppeteer");
	const { jsQR } = require("./html_qr");
	var browser = await puppeteer.launch({
		executablePath: !process.env.OS
			? "chromium-browser"
	...
	await page.waitForSelector("button._3e4VU");
	await loadChatBox("Mom");
	console.clear();
	console.log("Started Scanning");
	await page.evaluate(initQr);
	console.log(await page.evaluate(readChat));
})();
//fccciebbc
